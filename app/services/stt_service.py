...
#         # --- THE FIX: Hardware-Aware Local Fallback ---
#         if self.has_gpu:
#             actual_model = model_size
#             device = "cuda"
#             compute_type = "float16"
#             print(f">> [STT] GPU detected. Loading Bare-Metal STT '{actual_model}' engine on CUDA...")
#         else:
#             # If no GPU (laptop mode), force the ultra-fast base.en model on CPU
#             actual_model = "base.en"
#             device = "cpu"
#             compute_type = "int8"
#             print(f">> [STT] Laptop mode (No GPU). Loading ultra-fast '{actual_model}' engine on CPU...")
...
#                 # --- YOUR EXACT DUAL-GATE (0.85 Prob / 0.2 Vol) ---
#                 if prob_val > 0.85 and current_volume > 0.2: 
#                     if not is_recording:
#                         is_recording = True
#                         recording_buffer = list(pre_speech_buffer)
...
#     # --- THE FIX: Accept 'use_groq' safely so agent.py doesn't crash ---
#     async def transcribe_chunk(self, audio_bytes: bytes, use_groq: bool = False, **kwargs) -> str:
#         # SLICE BROWSER CHUNKS INTO PERFECT 1024-BYTE BITES
#         self.vad_chunk_buffer.extend(audio_bytes)
...
#         if not self.audio_queue.empty():
#             full_audio_bytes = self.audio_queue.get()
#             # It will process locally regardless of the 'use_groq' flag
#             text = await asyncio.to_thread(self._run_transcription, full_audio_bytes)
#             return text
#         return ""
...
#     def shutdown(self):
#         """Cleanly stops the background audio thread."""
#         print(">> [STT] Shutting down VAD worker...")
#         self.is_listening = False
#         if self.audio_thread.is_alive():
#             self.incoming_ws_queue.put(b'\x00' * 1024) 
